  ...
  try {
    ...
    const requestSchema = z.object({
      ...
      sections: z.array(z.object({
        title: z.string().min(1).max(24).trim(),
    ...
    const { conversationId, title, description, sections } = requestSchema.parse(await req.json());
    ...
    if (conversation.contact_number.includes('@lid') || conversation.contact_number.includes('@g.us')) {
      throw new Error('Não é possível enviar listas para listas de transmissão ou grupos via este método');
    }
    ...
    const response = await fetch(
      `${apiUrl}/message/sendList/${evolutionSettings.instance_name}`,
      {
        method: 'POST',
        headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to send list: ${response.statusText}`);
    }
    ...
    await supabase.from('messages').insert({
      ...
      list_data: { title, description, sections }
    });
