...
import { BarChart3, FileText, MessageSquare, TrendingUp, Brain, CheckCircle, XCircle, Clock, Zap, Database, AlertTriangle } from 'lucide-react';
    ...
    return (
        <Card>
            ...
            <CardContent>
                ...
                {trend && (
                    <p className={`text-xs mt-2 flex items-center gap-1 ${trendDown ? 'text-red-600' : 'text-green-600'}`}>
    ...
    const { data: stats, isLoading: statsLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            const avgConfidence = confidenceData && confidenceData.length > 0
                ? confidenceData.reduce((sum, q) => sum + (q.confidence_score || 0), 0) / confidenceData.length
                : 0;
                        ...
                        <div className="space-y-3">
                            <div>
                                <div className="flex items-center justify-between text-sm mb-1">
                                    ...
                                    <span className="font-medium text-green-600">{parseFloat(stats?.avgConfidence || '0') >= 70 ? '✓' : '-'}</span>
                                </div>
                                <Progress value={Math.min(parseFloat(stats?.avgConfidence || '0'), 100)} className="h-2" />
                            </div>
                            <div className="flex items-center justify-between text-xs text-muted-foreground">
                                <span>Consultas com baixa confiança</span>
                                <Badge variant={lowConfidenceQueries && lowConfidenceQueries.length > 5 ? "destructive" : "secondary"}>
            ...
            <div className="grid gap-4 md:grid-cols-2">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Últimas perguntas feitas à base de conhecimento
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        {queriesLoading ? (
                                ...
                                <div className="space-y-2">
                                    {topQueries.map((query, index) => (
                                        ...
                                        >
                                            ...
                                            <Badge
                                                variant={query.confidence_score >= 0.7 ? "default" : query.confidence_score >= 0.5 ? "secondary" : "destructive"}
                            ...
                            <div className="text-center py-8 text-muted-foreground">
                                ...
                                <p className="text-sm mt-2">
                                    Ative a Base de Conhecimento no Copiloto do chat.
                                </p>
                            </div>
                        )}
                    </CardContent>
            ...
            {lowConfidenceQueries && lowConfidenceQueries.length > 0 && (
                <Card className="border-amber-500/50">
                    <CardHeader>
                        ...
                        <CardDescription>
                            Perguntas que a IA teve dificuldade em responder - considere adicionar mais conteúdo sobre estes tópicos
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
