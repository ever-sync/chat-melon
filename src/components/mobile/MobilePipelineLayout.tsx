    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] pb-16">
      ...
      <div className="flex items-center justify-between p-4 border-b bg-background">
        ...
        <div className="flex-1 text-center">
          <h2 className="text-lg font-semibold truncate">{currentStage?.name || 'Pipeline'}</h2>
        ...
        <Button
          ...
          size="icon"
          onClick={() =>
            setCurrentStageIndex((prev) => Math.min((stages?.length || 0) - 1, prev + 1))
          }
        ...
        {stages?.map((_, index) => (
          <button
            ...
            className={cn(
              ...
              index === currentStageIndex ? 'w-8 bg-primary' : 'w-2 bg-muted'
            )}
          />
        ))}
      </div>
