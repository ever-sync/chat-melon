...
export function CampaignMetricsCards({ campaign }: CampaignMetricsCardsProps) {
  ...
  const replyRate = campaign.read_count > 0 
    ? ((campaign.reply_count / campaign.read_count) * 100).toFixed(1)
    : '0';

  const failRate = campaign.total_contacts > 0 
