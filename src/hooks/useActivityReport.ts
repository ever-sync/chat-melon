  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      messages?.forEach((msg) => {
        ...
        if (!dayMap.has(date))
          dayMap.set(date, { date, messages: 0, calls: 0, meetings: 0, proposals: 0, tasks: 0 });
      ...
      messages?.forEach((msg) => {
        if (!userMap.has(msg.user_id)) {
          userMap.set(msg.user_id, {
            userId: msg.user_id,
            userName: profileMap.get(msg.user_id) || 'Desconhecido',
            messages: 0,
            calls: 0,
            meetings: 0,
            proposals: 0,
      ...
      proposals?.forEach((prop) => {
        if (prop.created_by && !userMap.has(prop.created_by)) {
          userMap.set(prop.created_by, {
            userId: prop.created_by,
            userName: profileMap.get(prop.created_by) || 'Desconhecido',
            messages: 0,
            calls: 0,
            meetings: 0,
            proposals: 0,
      ...
      tasks?.forEach((task) => {
        if (!userMap.has(task.assigned_to)) {
          userMap.set(task.assigned_to, {
            ...
            userName: profileMap.get(task.assigned_to) || 'Desconhecido',
            messages: 0,
            calls: 0,
            meetings: 0,
            proposals: 0,
      ...
      dealActivities?.forEach((act) => {
        if (act.user_id && !userMap.has(act.user_id)) {
          userMap.set(act.user_id, {
            userId: act.user_id,
            userName: profileMap.get(act.user_id) || 'Desconhecido',
            messages: 0,
            calls: 0,
            meetings: 0,
            proposals: 0,
      ...
      proposals?.forEach((prop) =>
        detailedActivities.push({
          ...
          userName: profileMap.get(prop.created_by!) || 'Desconhecido',
          type: 'proposal',
          description: `Proposta ${prop.status === 'sent' ? 'enviada' : 'criada'}`,
        })
      );
      tasks?.forEach((task) =>
        detailedActivities.push({
