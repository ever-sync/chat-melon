  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const previousRevenueTotal =
        previousRevenue?.reduce((sum, d) => sum + (d.value || 0), 0) || 0;
      const revenueGrowth =
      ...
      const forecast =
        openDeals?.reduce((sum, d) => sum + ((d.value || 0) * (d.probability || 0)) / 100, 0) || 0;
      ...
      const avgClosingTime = wonDeals?.length
        ? wonDeals.reduce(
            (sum, d) => sum + differenceInDays(new Date(d.won_at!), new Date(d.created_at!)),
            0
          ) / wonDeals.length
        : 0;

      const avgClosingTimePrevious = previousWonDeals?.length
        ? previousWonDeals.reduce(
            (sum, d) => sum + differenceInDays(new Date(d.won_at!), new Date(d.created_at!)),
            0
          ) / previousWonDeals.length
        : 0;

      // Ticket médio
      const avgTicket = currentRevenue?.length ? currentRevenueTotal / currentRevenue.length : 0;
      const avgTicketPrevious = previousRevenue?.length
        ? previousRevenueTotal / previousRevenue.length
        : 0;
