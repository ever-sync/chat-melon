  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (filters.searchTerm) {
        query = query.or(`resource_name.ilike.%${filters.searchTerm}%,user_email.ilike.%${filters.searchTerm}%`);
      }
...
export function useLogAuthEvent() {
  return useMutation({
    mutationFn: async ({
      ...
      eventType: "login" | "logout" | "failed_login" | "password_reset" | "2fa_enabled" | "2fa_disabled";
      userId?: string;
      userEmail?: string;
      ipAddress?: string;
      userAgent?: string;
