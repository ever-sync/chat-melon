...
export const useAnalytics = (months: number = 6, pipelineId?: string) => {
  ...
  const { data: metrics, isLoading: metricsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      let query = supabase.from('deals').select('*').eq('company_id', companyId);
      ...
      const avgPipelineTime =
        wonDeals.length > 0 ? Math.floor(totalPipelineTime / wonDeals.length) : 0;
  ...
  const { data: forecastData, isLoading: forecastLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (needed > 0) {
        alerts.push(
          `Você precisa de ${new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(needed)} para bater a meta`
        );
      }
      ...
      const accuracyPercent =
        previousForecast > 0 ? (previousRealized / previousForecast) * 100 : 0;
  ...
  return {
    ...
    forecastData,
    isLoading:
      metricsLoading || revenueLoading || funnelLoading || rankingLoading || forecastLoading,
  };
};
