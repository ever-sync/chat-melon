...
export function useCadenceAnalytics(cadenceId: string | undefined) {
    const analyticsQuery = useQuery({
        ...
        queryFn: async () => {
            ...
            const avgStep = enrollments?.length > 0
            ...
            return {
                ...
                avgStep,
                replyRate: cadence.total_enrolled > 0
                    ? (cadence.total_replied / cadence.total_enrolled) * 100
                    : 0,
                conversionRate: cadence.total_enrolled > 0
                    ? (cadence.total_converted / cadence.total_enrolled) * 100
                    : 0,
            };
        },
        enabled: !!cadenceId,
